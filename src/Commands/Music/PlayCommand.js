  ...
  async exec(msg, { query }) {
    try {
      ...
      if (MusicTracks.loadType === 'NO_MATCHES') {
        return msg.channel.send({ embeds: [CreateEmbed('warn', '⛔ | No result found.')] });
      }

      if (MusicTracks.loadType === 'LOAD_FAILED') {
        return msg.channel.send({ embeds: [CreateEmbed('warn', '⛔ | An error occured when loading the track.')] });
      }
