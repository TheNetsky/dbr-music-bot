...
module.exports = class SkipCommand extends Command {
  ...
  async exec(msg) {
    try {
      ...
      GuildPlayers.stop();
      return msg.channel.send({ embeds: [this.client.utils.CreateEmbed().setDescription('👌 | Skipped current track')] });
    } catch (e) {
  ...
  async executeSlash(interaction) {
    try {
      ...
      GuildPlayers.stop();
      return interaction.editReply({ embeds: [this.client.utils.CreateEmbed().setDescription('👌 | Skipped current track')] });
    } catch (e) {
      this.client.logger.error(e.message);
