...
module.exports = class PauseCommand extends Command {
  ...
  async executeSlash(interaction) {
    try {
      ...
      if (!GuildPlayers) {
        return interaction.editReply({ embeds: [CreateEmbed('info', '⛔ | There no music playing in this guild')] });
      }
      
      if (!interaction.member.voice.channelId) {
        return interaction.editReply({ embeds: [CreateEmbed('warn', '⛔ | you must join voice channel to do this.')] });
      }

      if (interaction.member.voice.channelId !== GuildPlayers.voiceChannel) {
        return interaction.editReply({ embeds: [CreateEmbed('warn', '⛔ | you must join voice channel same as me to do this.')] });
      }
